...
double SysInfoWindowsImpl::memoryUsed(){
  ...
  return (double)memoryPhysicalUsed / (double)memoryStatus.ullTotalPhys * 100.00;
}
...
double SysInfoWindowsImpl::cpuLoadAverage(){
  ...
  QVector<qulonglong> seondSample = cpuRawData();
  mCpuLoadLastValues = secondSample;

  qulonglong currentIdle = secondSample[0] - firstSample[0];
  qulonglong currentKernel = secondSample[1] - firstSample[1];
  qulonglong currentUser = secondSample[3] - firstsample[3];
  qulonglong currentSystem = currentKernel + currentUser;

  double percent = (currentSystem - currentIdle) * 100.00 / currentSystem;
  return qBound(0.0, percent, 100.00);
